...
export default class TrainerMultiplex implements Multiplex {
  ...
  connect (): void {
    ...
    this.deck.on('slidechanged', () => this.postState())
    this.deck.on('fragmentshown', () => this.postState())
    this.deck.on('fragmenthidden', () => this.postState())
    this.deck.on('overviewhidden', () => this.postState())
    this.deck.on('overviewshown', () => this.postState())
    this.deck.on('paused', () => this.postState())
    this.deck.on('resumed', () => this.postState())

    // Monitor events that should be broadcasted to other presentations
    this.deck.on('quiz-show-responses', (event) => this.postEvent(event))
    this.deck.on('quiz-lock', (event) => this.postEvent(event))
    // on locking, also send state to force users to current slide
    this.deck.on('quiz-lock', () => this.postState())
    this.deck.on('quiz-unlock', (event) => this.postEvent(event))
    this.deck.on('quiz-reset', (event) => this.postEvent(event))
    // events to show/hide qrcodes
    this.deck.on('qrcode-show', (event) => this.postEvent(event))
    this.deck.on('qrcode-hide', (event) => this.postEvent(event))
